...
router.get("/:id", (req, res) => {
  userFns.getUserById(req.params.id).then((user) => {
    if (!helperFns.adminOrCustomer(user.id)) {
      homeFns.getHome().then((food_items) => {
        return res.render("index", { user, food_items });
      });
    } else {
      restaurantFns.getNewestOrder().then((carts) => {
...
router.post("/:id", (req, res) => {
  const response = {
    time: helperFns.minutesFromRestaurant(req.body.time),
